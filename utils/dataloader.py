...
import numpy as np
import nltk
nltk.data.path.append("/home2/jxy/ENDEF-SIGIR2022-main/ENDEF_en/models/punkt/PY3")
...
def word2input(texts, max_len):
    ...
    local_model_pathh = '/home2/jxy/ENDEF-SIGIR2022-main/ENDEF_en/models/bert-base-uncased'
    tokenizer = BertTokenizer.from_pretrained(local_model_pathh)
    token_ids = []
    ...
    if random_num <= 50:
        for item in entity_list:
            ...
            if random_num <= int(aug_prob * 100):
                content = content.replace(item["entity"], '[MASK]')#遮罩
            elif random_num <= int(2 * aug_prob * 100):
                content = content.replace(item["entity"], '')#丢弃
            else:
...
def get_dataloader(path, max_len, batch_size, shuffle, use_endef, aug_prob):
    data_list = json.load(open(path, 'r',encoding='utf-8'))#self.config['root_path'] + 'train.json'
    df_data = pd.DataFrame(columns=('content','label'))#创建一个空的 DataFrame，具有名为 content 和 label 的两列
    print("1")
...
def get_dataloader1(path, max_len, batch_size, shuffle, use_endef, aug_prob):
    data_list = json.load(open(path, 'r',encoding='utf-8'))#self.config['root_path'] + 'train.json'
    df_data = pd.DataFrame(columns=('content','label'))#创建一个空的 DataFrame，具有名为 content 和 label 的两列
    print("1")
    for item in data_list:
